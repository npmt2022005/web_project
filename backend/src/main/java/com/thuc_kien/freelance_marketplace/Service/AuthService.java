...
public class AuthService {
    ...
    public void forgotPassword(String identifier) {
        ...
        User user = userRepo.findByUsernameOrEmailOrPhone(itf, itf, itf)
                .orElseThrow(() -> new AppException("Tài khoản, Email không tồn tại"));
        ...
        if (itf.matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
            if (user.getEmail() != null && !user.getEmail().isEmpty()) {
                // Lưu OTP theo Email để đồng bộ lúc xác thực
                otpService.saveOtp(user.getEmail(), otp);

                // Gửi Mail
                emailService.sendOtpEmail(user.getEmail(), otp);
            }
            // 4. Nếu không có Email thì kiểm tra đến Số điện thoại
            else if (user.getPhone() != null && !user.getPhone().isEmpty()) {
                // Lưu OTP theo Số điện thoại
                otpService.saveOtp(user.getPhone(), otp);

                // Gửi SMS
                smsService.sendOtpSms(user.getPhone(), otp);
            }
            // 5. Trường hợp hy hữu: Tài khoản tồn tại nhưng không có cả Email lẫn SĐT
            else {
                throw new AppException("Tài khoản này không có thông tin liên lạc (Email/SĐT) để nhận mã OTP");
            }
        }
    ...
    public void verifyOtp(String loginId, String otp) {
        String itf = loginId.trim();
        User user = userRepo.findByUsernameOrEmailOrPhone(itf, itf, itf)
                .orElseThrow(() -> new AppException("Tài khoản không tồn tại trên hệ thống"));
        String redisKey = itf;
        if (user.getEmail() != null && !user.getEmail().isEmpty()) {
            redisKey = user.getEmail();
        } else if (user.getUsername() != null && !user.getUsername().isEmpty()) {
            redisKey = user.getUsername();
        }

        String savedOtp = otpService.getOtp(redisKey);
        if (savedOtp == null) {
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        User user = userRepo.findByUsernameOrEmailOrPhone(
                identifier, identifier, identifier).orElseThrow(() -> new AppException("Người dùng không tồn tại"));
